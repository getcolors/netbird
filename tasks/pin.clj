...
(def path "skills/package-netbird-green/green")
(def rx #"\(def \^:private netbird-sha (nil|\"[0-9a-f]{40}\")\)")
...
(let [dirty (git "status" "--porcelain") sha (git "rev-parse" "HEAD") remotes (git "branch" "-r" "--contains" sha)]
  (cond (seq dirty) (do (binding [*out* *err*] (println "netbird working tree is dirty; commit before pinning")) (System/exit 2))
        ...
        :else (let [s (slurp path) n (str/replace s rx (str "(def ^:private netbird-sha \"" sha "\")"))]
                (spit path n) (println "pinned netbird launcher to" (subs sha 0 7)))))
