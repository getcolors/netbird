...
for variant in colors optout; do
  ...
  (cd "$root/green" && NETBIRD_LIB_ROOT="$root" ./green build -f "$fixture" >/dev/null)
  ...
  # The five operator secrets must reach the host as Ansible lookups resolved
  ...
  for par in NETBIRD_BOOTSTRAP_PASSWORD NETBIRD_AUTHENTIK_BOOTSTRAP_PASSWORD \
             NETBIRD_BACKUP_RECOVERY_KEY NETBIRD_BACKUP_R2_ACCESS_KEY_ID \
             NETBIRD_BACKUP_R2_SECRET_ACCESS_KEY; do
    if ! grep -q "lookup('env','COLORS_PAR_$par')" "$actual/netbird-ansible/main.yml"; then
      echo "golden: $profile no longer renders COLORS_PAR_$par as a lookup" >&2; exit 1
    fi
  done

  # The three server secrets are substituted on the host at install time. If a
  # real value ever reaches the rendered config, the next `bb golden:accept`
  ...
  for ph in __RELAY_AUTH_SECRET__ __SESSION_COOKIE_ENCRYPTION_KEY__ __DATASTORE_ENCRYPTION_KEY__; do
    if ! grep -q "$ph" "$actual/netbird-ansible/config.yaml"; then
  ...
  if grep -rEq '([0-9]{1,3}\.){3}[0-9]{1,3}' "$actual/netbird-ansible-local"; then
