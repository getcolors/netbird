...
provider, on one VM managed by `colors-compute`. Package policy requests
22/80/443 TCP and 3478 UDP; its DNS stage manages two unproxied Cloudflare `A` records. Ansible
...
The compute library owns generated keys and provider registration. The package owns the locked profile SSH updater. The alias is removed before compute destruction; key cleanup occurs inside the library after successful destruction. External private paths are explicit Ansible inputs, and external aliases do not add IdentityFile. Build and dry-run use deterministic plans without inspecting local SSH files.

## Compute ownership and policy

`colors-compute` owns singleton VM creation, provider selection, name resolution, remote state, and lifecycle coordination. Package requirements preserve TCP22/80/443, the configured UDP STUN port and its source list, and `ipv6:false`. SSH sources must be nonempty; HTTP and STUN sources may be empty. The neutral `netbird-ssh-sources`, `netbird-http-sources`, and `netbird-stun-sources` options override library-resolved legacy provider options. Unsupported capabilities fail explicitly; adding support belongs in the library.

R2/S3 state lives under `<profile>/compute/shared.tfstate` and `<profile>/compute/nodes/0.tfstate`, with a deployment journal. Existing `<profile>/netbird-infrastructure.tfstate` is refused for explicit operator migration; unreadable state never becomes an invented address. DNS remains separate. Names follow the library's common safe-name contract, so legacy dotted overrides require revision during migration.

Create runs compute, SSH alias, DNS, application convergence, then acceptance. Delete first inspects owned compute, performs application shutdown/final backup, removes DNS and the SSH alias, then destroys compute. Authentik and NetBird generated credentials, federation bootstrap, encrypted backup, and acceptance scripts remain package-owned and unchanged.
