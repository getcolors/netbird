...
if [[ -z $(local_pat) ]]; then
  ...
  if setup_out=$(curl -fsS -X POST "$API/setup" -H 'content-type: application/json' \
    ...
    log "  in the dashboard as breakglass@example.com and writing it to"
...
if [[ -n $(cat "$SECRETS/pat" 2>/dev/null || true) ]] \
  ...
  log "signing in as admin@example.com through Authentik"
  token=$(/usr/local/sbin/netbird-federated-login \
            "$idp" "netbird.example.com" "authentik.example.com" \
...
owner=$(api "$PAT" GET /users | jq -r --arg e "admin@example.com" \
  '.[] | select(.email==$e and .is_current==true) | .role' | head -1)
if [[ $owner != owner ]]; then
  log "FATAL: admin@example.com is '$owner' in the federated account, not owner"
  exit 1
fi
log "federated account owned by admin@example.com"
