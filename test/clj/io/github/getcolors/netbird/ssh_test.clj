(ns io.github.getcolors.netbird.ssh-test
...
(defn- with-home
  ...
  [f]
  (let [home (str (fs/create-temp-dir {:prefix "netbird-ssh"}))]
    (try
  ...
  (with-home
    (fn [home]
      (let [opts (ssh/with-machine-key (assoc (fixture) :green/event :create))]
        (is (= (str (io/file home ".ssh" "netbird-fixture")) (:ssh-private-key-path opts)))
        (is (= (str (io/file home ".ssh" "netbird-fixture.pub")) (:ssh-public-key-path opts)))))))
  ...
  (with-home
    (fn [home]
      (let [opts (ssh/ensure-key! (assoc (fixture) :green/event :create) (constantly nil))
            prv (io/file home ".ssh" "netbird-fixture")
            pub (io/file home ".ssh" "netbird-fixture.pub")]
        ...
        (testing "ed25519, no passphrase, profile-named comment"
          ...
          (is (str/includes? (slurp pub) "netbird-fixture managed by Colors")))
...
(deftest converge-reuses-an-existing-key
  (with-home
    (fn [home]
      (write! (str (io/file home ".ssh" "netbird-fixture")) "private")
      (write! (str (io/file home ".ssh" "netbird-fixture.pub")) "ssh-ed25519 AAAA test")
      (let [opts (ssh/ensure-key! (assoc (fixture) :green/event :create)
                                  (constantly {:ip "192.0.2.10"}))]
        (is (not (contains? opts :green/err)))
        (is (= "private" (slurp (io/file home ".ssh" "netbird-fixture")))
...
(deftest a-key-without-state-is-never-overwritten
  (with-home
    (fn [home]
      (let [prv (str (io/file home ".ssh" "netbird-fixture"))]
  ...
  (with-home
    (fn [_]
      (let [opts (-> (assoc (fixture) :green/event :create
                     ...
                     (preflight [{:id "abc" :name "netbird-fixture" :public "ssh-ed25519 AAAA"}]))]
        (is (not (contains? opts :green/err)))))))

(deftest preflight-refuses-our-leftover-key
  (with-home
    (fn [home]
      (write! (str (io/file home ".ssh" "netbird-fixture.pub")) "ssh-ed25519 AAAA comment")
      (let [opts (-> (assoc (fixture) :green/event :create)
                     ssh/with-machine-key
                     (preflight [{:id "abc" :name "netbird-fixture" :public "ssh-ed25519 AAAA"}]))]
        (is (= 1 (:green/exit opts)))
...
(deftest preflight-refuses-a-foreign-key-and-says-do-not-delete-it
  (with-home
    (fn [home]
      (write! (str (io/file home ".ssh" "netbird-fixture.pub")) "ssh-ed25519 OURS comment")
      (let [opts (-> (assoc (fixture) :green/event :create)
                     ssh/with-machine-key
                     (preflight [{:id "abc" :name "netbird-fixture" :public "ssh-ed25519 THEIRS"}]))]
        (is (= 1 (:green/exit opts)))
...
(deftest delete-removes-the-keypair
  (with-home
    (fn [home]
      (write! (str (io/file home ".ssh" "netbird-fixture")) "private")
      (write! (str (io/file home ".ssh" "netbird-fixture.pub")) "public")
      (ssh/cleanup-step (assoc (fixture) :green/event :delete :ssh-keygen true))
      (is (not (.exists (io/file home ".ssh" "netbird-fixture"))))
      (is (not (.exists (io/file home ".ssh" "netbird-fixture.pub"))))
...
(deftest cleanup-is-inert-on-create-and-in-opt-out-mode
  (with-home
    (fn [home]
      ...
      (is (.exists (io/file home ".ssh" "netbird-fixture")))
