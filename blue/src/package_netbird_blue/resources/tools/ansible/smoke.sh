...
disco=$(curl -fsS "https://<{ netbird-authentik-host }>/application/o/netbird/.well-known/openid-configuration")
...
grant=$(curl -fsS -X POST "$dev_ep" -d "client_id=<{ netbird-oidc-client-id }>" -d "scope=openid profile email" || true)
