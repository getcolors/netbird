...
from package_netbird_blue import ssh_config, tools, workflow

# §2 the alias and the identity file


def test_alias_is_the_profile():
    assert ssh_config.host_alias(fixture()) == "netbird-fixture"
...
def test_identity_file_keeps_the_tilde(tmp_path, monkeypatch):
    ...
    assert ssh_config.identity_file(fixture()) == "~/.ssh/netbird-fixture"
...
def test_the_marker_is_the_alias_alone():
    ...
    # too would repeat it: "# BEGIN netbird netbird-vultr".
    assert ssh_config.begin_marker("netbird-vultr") == "# BEGIN netbird-vultr ANSIBLE MANAGED BLOCK"
    assert ssh_config.end_marker("netbird-vultr") == "# END netbird-vultr ANSIBLE MANAGED BLOCK"
...
def test_a_foreign_stanza_is_found():
    lines = ["Host other", "    HostName 192.0.2.1", "", "Host netbird-fixture"]
    assert ssh_config.foreign_stanza_line(lines, "netbird-fixture") == 4


def test_our_own_block_is_not_foreign():
    alias = "netbird-fixture"
...
def test_a_block_under_a_retired_marker_is_foreign():
    ...
    alias = "netbird-vultr"
    lines = [f"# BEGIN netbird {alias} ANSIBLE MANAGED BLOCK",
             f"Host {alias}",
             f"# END netbird {alias} ANSIBLE MANAGED BLOCK"]
    assert ssh_config.foreign_stanza_line(lines, alias) == 2


def test_a_multi_pattern_host_line_counts():
    assert ssh_config.foreign_stanza_line(["Host web netbird-fixture db"], "netbird-fixture") == 1


def test_an_unrelated_file_is_left_alone():
    assert ssh_config.foreign_stanza_line(["Host build", "Host netbird-other"],
                                          "netbird-fixture") is None
...
def test_the_local_play_renders_no_address():
    ...
    assert data["ssh-config-identity-file"] == "~/.ssh/netbird-fixture"
...
def test_the_local_stage_renders_three_files():
    ...
    assert all("netbird-ansible-local" in t for t in targets)
...
def test_delete_removes_the_block_before_the_destroy():
    ...
    assert workflow.wire_fn("netbird/dns", delete)[1:] == ("netbird/ssh-config",)
    assert workflow.wire_fn("netbird/ssh-config", delete)[1:] == ("netbird/infrastructure",)
