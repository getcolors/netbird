...
from package_netbird_blue import ssh
...
def test_real_events_render_the_real_path(home):
    ...
    assert opts["ssh-private-key-path"] == str(home / ".ssh" / "netbird-fixture")
    assert opts["ssh-public-key-path"] == str(home / ".ssh" / "netbird-fixture.pub")
...
async def test_first_create_generates_the_keypair(home):
    ...
    prv = home / ".ssh" / "netbird-fixture"
    pub = home / ".ssh" / "netbird-fixture.pub"
    ...
    assert "netbird-fixture managed by Colors" in pub.read_text()
...
async def test_converge_reuses_an_existing_key(home):
    write(home / ".ssh" / "netbird-fixture", "private")
    write(home / ".ssh" / "netbird-fixture.pub", "ssh-ed25519 AAAA test")
    opts = await ssh.ensure_key({**fixture(), "blue/event": "create"},
                                state({"ip": "192.0.2.10"}))
    assert "blue/err" not in opts
    assert (home / ".ssh" / "netbird-fixture").read_text() == "private", \
...
def test_preflight_passes_when_the_account_key_is_ours(home):
    opts = preflight(
        ...
        [{"id": "abc", "name": "netbird-fixture", "public": "ssh-ed25519 AAAA"}])
    assert "blue/err" not in opts


def test_preflight_refuses_our_leftover_key(home):
    write(home / ".ssh" / "netbird-fixture.pub", "ssh-ed25519 AAAA comment")
    opts = preflight(ssh.with_machine_key({**fixture(), "blue/event": "create"}),
                     [{"id": "abc", "name": "netbird-fixture", "public": "ssh-ed25519 AAAA"}])
    assert opts["blue/exit"] == 1
...
def test_preflight_refuses_a_foreign_key_and_says_do_not_delete_it(home):
    write(home / ".ssh" / "netbird-fixture.pub", "ssh-ed25519 OURS comment")
    opts = preflight(ssh.with_machine_key({**fixture(), "blue/event": "create"}),
                     [{"id": "abc", "name": "netbird-fixture", "public": "ssh-ed25519 THEIRS"}])
    assert opts["blue/exit"] == 1
...
def test_delete_removes_the_keypair(home):
    write(home / ".ssh" / "netbird-fixture", "private")
    write(home / ".ssh" / "netbird-fixture.pub", "public")
    ssh.cleanup_step({**fixture(), "blue/event": "delete", "ssh-keygen": True})
    assert not (home / ".ssh" / "netbird-fixture").exists()
    assert not (home / ".ssh" / "netbird-fixture.pub").exists()
...
def test_cleanup_is_inert_on_create_and_in_opt_out_mode(home):
    ...
    assert (home / ".ssh" / "netbird-fixture").exists()
