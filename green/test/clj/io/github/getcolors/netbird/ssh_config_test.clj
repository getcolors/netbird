(ns io.github.getcolors.netbird.ssh-config-test
...
(deftest alias-is-the-profile
  (is (= "netbird-fixture" (ssh-config/host-alias (fixture)))))
...
(deftest identity-file-keeps-the-tilde
  ...
  (is (= "~/.ssh/netbird-fixture" (ssh-config/identity-file (fixture))))
...
(deftest the-marker-is-the-alias-alone
  ...
  (is (= "# BEGIN netbird-vultr ANSIBLE MANAGED BLOCK"
         (ssh-config/begin-marker "netbird-vultr")))
  (is (= "# END netbird-vultr ANSIBLE MANAGED BLOCK"
         (ssh-config/end-marker "netbird-vultr"))))

;; §5 never adopt

(deftest a-foreign-stanza-is-found
  (let [lines ["Host other" "    HostName 192.0.2.1" "" "Host netbird-fixture"]]
    (is (= 4 (ssh-config/foreign-stanza-line lines "netbird-fixture")))))

(deftest our-own-block-is-not-foreign
  (let [alias "netbird-fixture"
...
(deftest a-block-under-a-retired-marker-is-foreign
  ;; The superseded `# BEGIN netbird <alias>` marker is gone, so a block
  ...
  (let [alias "netbird-vultr"
        lines [(str "# BEGIN netbird " alias " ANSIBLE MANAGED BLOCK")
               (str "Host " alias)
               (str "# END netbird " alias " ANSIBLE MANAGED BLOCK")]]
    (is (= 2 (ssh-config/foreign-stanza-line lines alias)))))

(deftest a-multi-pattern-host-line-counts
  (is (= 1 (ssh-config/foreign-stanza-line ["Host web netbird-fixture db"]
                                           "netbird-fixture"))))

(deftest an-unrelated-file-is-left-alone
  (is (nil? (ssh-config/foreign-stanza-line ["Host build" "Host netbird-other"]
                                            "netbird-fixture"))))
...
(deftest the-local-play-renders-no-address
  ...
  (let [data (tools/ansible-local-data (assoc (fixture) :ip "203.0.113.7"))]
    ...
    (is (= "~/.ssh/netbird-fixture" (:ssh-config-identity-file data)))))
...
(deftest the-local-stage-renders-three-files
  (let [targets (map #(str (:target %)) (tools/ansible-local-specs (fixture)))]
    ...
    (is (every? #(str/includes? % "netbird-ansible-local") targets))))
