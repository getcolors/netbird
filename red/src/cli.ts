...
import { netbirdWorkflow } from "./workflow.ts";
...
export const usage =
  ...
  "  create    provision and verify the NetBird control plane\n" +
...
export async function run(...args: string[]): Promise<Opts> {
  ...
  if (lifecycleCommands.includes(command)) {
    return runCli(netbirdWorkflow, withFile);
  }
...
export async function exec(args: string[] = Bun.argv.slice(2)): Promise<never> {
  if (lifecycleCommands.includes(args[0] ?? "")) {
    return execCli(netbirdWorkflow, defaultArgs(args));
  }
